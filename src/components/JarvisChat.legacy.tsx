...
const JarvisChat = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "1", text: "Boa noite, senhor. Eu sou JARVIS, seu assistente inteligente. Como posso ajudá-lo hoje?", isUser: false, timestamp: new Date() },
  ]);
  ...
  const handleTypingComplete = (messageId: string) => { if (typingMessageId === messageId) setTypingMessageId(null); };
...
export default JarvisChat;
      ...
      // Mostra resposta inicial
      const assistantId = (Date.now() + 1).toString();
      setMessages((prev) => [...prev, { id: assistantId, text: replyText, isUser: false, timestamp: new Date() }]);
      setTypingMessageId(assistantId);

      // Processa ações, pedindo confirmação quando necessário
      for (const action of actions) {
        try {
          if (action.requires_confirmation) {
            // PoC: usa window.confirm — trocar por modal customizado para melhor UX
            const confirmed = window.confirm(`Confirmar ação: ${action.command} em ${action.target}?`);
            if (!confirmed) {
              setMessages((prev) => [...prev, { id: `not-confirmed-${action.id}`, text: `Ação ${action.command} em ${action.target} não confirmada.`, isUser: false, timestamp: new Date() }]);
              continue;
          ...
          setMessages((prev) => [...prev, { id: `action-error-${action.id}`, text: `Falha ao executar ação ${action.command}: ${actErr?.message || actErr}`, isUser: false, timestamp: new Date() }]);
        }
      }
      ...
      setIsTyping(false);
      setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), text: "Desculpe, senhor. Estou com dificuldades técnicas no momento.", isUser: false, timestamp: new Date() }]);
      toast({ title: "Erro de Comunicação", description: error instanceof Error ? error.message : "Erro desconhecido", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-jarvis-deep-black">
            ...
            <div className={cn("max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl rounded-lg p-4 jarvis-chat-message", message.isUser ? "bg-jarvis-gold text-jarvis-deep-black ml-auto" : "bg-jarvis-surface-elevated text-jarvis-gold border border-jarvis-gold/30")}>
              <p className="text-sm leading-relaxed">
                {!message.isUser && typingMessageId === message.id ? (
                  <TypingEffect text={message.text} speed={25} onComplete={() => handleTypingComplete(message.id)} />
                ) : (
                  message.text
                )}
              </p>
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="bg-jarvis-surface-elevated border border-jarvis-gold/30 rounded-lg p-4 jarvis-chat-message">
              ...
              <span className="text-xs text-jarvis-gold-light mt-2 block">JARVIS está pensando...</span>
            </div>
          </div>
        )}
        <div ref={messagesEndRef} />
      ...
      <div className="border-t border-jarvis-gold/20 p-4 bg-jarvis-surface">
        <div className="max-w-4xl mx-auto flex space-x-2">
          <div className="flex-1 relative">
            <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={handleKeyPress} placeholder="Pergunte qualquer coisa ao JARVIS..." className="bg-jarvis-surface-elevated border-jarvis-gold/30 text-jarvis-gold placeholder:text-jarvis-gold/50 focus:border-jarvis-gold focus:ring-jarvis-gold/20 pr-10" />
            <Button variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 text-jarvis-gold hover:bg-jarvis-gold/10 h-8 w-8">
              <Mic className="w-4 h-4" />
            </Button>
          </div>
          <Button onClick={sendMessage} disabled={!input.trim() || isTyping} className="bg-jarvis-gold text-jarvis-deep-black hover:bg-jarvis-gold-light disabled:opacity-50 disabled:cursor-not-allowed jarvis-glow">
