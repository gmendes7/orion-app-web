...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🌤️ Buscando ${type} para: ${type || `${lat}, ${lon}`}`);
    ...
    return new Response(
      JSON.stringify({
        ...
        fallback:
          "Não foi possível obter os dados meteorológicos no momento. Verifique se a cidade foi digitada corretamente.",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
